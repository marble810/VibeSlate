...
All optional password auth and LAN HTTPS settings now live directly in `docker/docker-compose.yml`. There is no separate `init` step.

## Required Fields

Edit `docker/docker-compose.yml` and fill the provider credentials you actually use:
...
Print paste-ready YAML from the host's Codex auth state:
...
- `./docker/GetCodexAuthInfo.sh --format env`
- `./docker/GetCodexAuthInfo.ps1`

## Optional Password Auth

Password auth is controlled entirely by these compose fields:
...
- `TLS_ENABLED: "true"` in this compose file
- you are serving VibeSlate behind your own HTTPS reverse proxy

To generate the hash, use any Argon2id-compatible tool. If you already have a Bun-capable checkout, this is enough:
...
LAN HTTPS is also controlled in compose:
...
- Password auth enabled but login loops on plain HTTP: verify `AUTH_COOKIE_SECURE` is `"false"`.
- Password auth behind HTTPS: switch `AUTH_COOKIE_SECURE` to `"true"`.
