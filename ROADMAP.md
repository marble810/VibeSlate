...
| M7 | Docker | 单容器部署，`.env` 配置 provider/auth/TLS/user settings，`docker:up`/`docker:smoke` 辅助验证 |
...
- [x] 公共部署路径采用 `docker/.env.example -> docker/.env` + `docker-compose.example.yml -> docker-compose.yml`
- [x] 所有 provider/auth/TLS/user settings 通过本地 `docker/.env` 设置
...
- [x] Docker 通过 `docker/.env` 读取 auth 配置
- [ ] 支持首次启动初始化密码与后续修改密码
...
- [x] Docker 公开部署改为 `docker/.env.example` + `docker/docker-compose.example.yml` 双模板复制流程
- [x] 增加纯 shell / PowerShell 的 Codex 凭证打印脚本
- [x] 统一 Docker Quick Start、helper 输出与 `.env` 配置文案为单一路径
...
- [ ] `.env` 收敛后，clean checkout 公开仓库 + 公开镜像 Docker-only 主路径需要按 `.env` 层 auth/TLS 配置重新验收
